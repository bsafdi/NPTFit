...
#
# This version operates for non-integer counts
#
###############################################################################
...
def log_like_poissonian(double[::1] pt_sum_compressed, double[::1] data):
    ...
    return log_like_poissonian_int_non_int(pt_sum_compressed, data)

@cython.boundscheck(False)
@cython.wraparound(False)
@cython.cdivision(True)
@cython.initializedcheck(False)
cdef double log_like_poissonian_int_non_int(double[::1] pt_sum_compressed,
                                    double[::1] data):
    ...
    with nogil:
        for p in range(npix_roi):
            ll += -pt_sum_compressed[p] + data[p]*log(pt_sum_compressed[p]) - lgamma( data[p] + 1. )
    return ll
